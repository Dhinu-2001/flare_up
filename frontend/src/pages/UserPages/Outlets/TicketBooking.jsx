...
export default function TicketBooking() {
  ...
  const makePayment = async (e) => {
    ...
    try {
      ...
      const stripe = await loadStripe('pk_test_51QOwBVEoHTcQ6zGjLjuWXuhGMGa4bKAFjM3iwkcRMaEMzq1GSBFuSx1o1Llv0sZbFFMczTlY2j5p6mLwtOqxD5aH0056InV0d3');
      const payment_data = {
        'user_id': user_id,
      ...
      const response = await axiosInstance.post('/registration_api/create-checkout-session/', payment_data, {
        headers: {
          ...
          <Form {...form} >
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <p className='text-sm font-bold'>Currently {data.event_data.participant_capacity} tickets available.</p>
              ...
              <div className="text-xl font-bold">
                <table className='border-collapse w-full text-sm'>
                  <tbody>
                    ...
                    <tr>
                      <td>Ticket Amount:</td>
                      <td className='text-right'>{data.event_data.payment_required ? (data.event_data.ticket_price) : ('free')}</td>
                    </tr>
                    <br />
                    <tr>
                      <td>Total:</td>
              ...
              <Button
                onClick={makePayment}
                className='rounded-none bg-sky-500' type="button">Book Tickets</Button>
            </form>
          </Form>
        </div>
      </main >
